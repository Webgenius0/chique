...
const SideBar = ({ isOpen = false }) => {
    ...
    return (
        ...
        `}>
            ...
            {/* NAV ITEMS */}
            <nav className="w-full pl-6 py-8 h-full flex flex-col justify-start gap-3 overflow-y-auto ">
                {navItems.map((item) => {
                    ...
                    return (
                        <Link
                            key={item.path}
                            href={item.path}
                            className={` w-full rounded-tl-lg text-xl rounded-bl-lg px-4 py-3 flex justify-start items-center gap-3 transition-colors duration-300 ${isActive
